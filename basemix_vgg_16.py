...
def get_init_values_for_pruned_layers(prune_scopes, shorten_scopes, kept_percentage):
    ...
    with graph.as_default():
        ...
        variables_init_value, _ = get_pruned_kernel_matrix(sess, prune_scopes, shorten_scopes, kept_percentage)

    return variables_init_value, None 
...
def main(_):
    ...
    if FLAGS.last_conv_pruned:
        FLAGS.train_dir = FLAGS.train_dir + '_last_conv_pruned'
    results_dir = os.path.join(FLAGS.train_dir, 'id'+str(config_id)) 
    ...
    if not (FLAGS.continue_training and tf.train.latest_checkpoint(train_dir)):
        ...
        variables_init_value,_ = get_init_values_for_pruned_layers(prune_scopes, shorten_scopes, config)
