...
def get_init_values_for_pruned_layers(prune_info, net_name_scope):
    print('HG: start to get initial values for pruned blocks')
    graph = tf.Graph()
    with graph.as_default():
        ...
        with tf.Session() as sess:
            ...
            variables_init_value = get_pruned_kernel_matrix(sess, prune_info, net_name_scope)
    # # remove graph 
    # del graph 
...
def main(_):
    ...
    if (not FLAGS.continue_training) or (not tf.train.latest_checkpoint(train_dir)):
        ...
        variables_init_value = get_init_values_for_pruned_layers(prune_info, net_name_scope_checkpoint)
    else:
    ...
    with tf.Graph().as_default():
        ...
        logits_train, end_points = network_fn_pruned(images, 
                                                     is_training=True, 
        ...
        # pprint(end_points)
        # return 
        # pprint(get_model_variables_within_scopes())
        # return 
        ...
        # use activation map loss if no logits loss and no cross entropy loss 
        if not FLAGS.include_cross_entropy and FLAGS.logits_loss_lambda==0:
            last_block_name = block_names[-1]
            outputs = end_points_teacher[last_block_name]
            outputs_pruned = end_points[last_block_name]
