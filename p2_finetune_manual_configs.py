...
tf.app.flags.DEFINE_string(
    'configs_path', './block_sensitivity_study/manual_configs.txt',
...
def main(_):
    ...
    print("kept_percentages_dict:", kept_percentages_dict)
    ...
    # prepare file system  
    results_dir = os.path.join(FLAGS.train_dir, "id"+str(FLAGS.config_id)) #+'_'+str(FLAGS.max_number_of_steps))
            ...
            if (not FLAGS.continue_training) or (not tf.train.latest_checkpoint(train_dir)):
                ...
                for scope_kept_percentage, train_scopes in train_scopes_dict.items():
                    ...
                    checkpoint_path = os.path.join(
                        ...
                        kept_percentages_dict[scope_kept_percentage][0], 'train')
                    #    'model.ckpt-'+str(FLAGS.local_train_steps))

                    variables_to_train = {re.sub(net_name_scope_pruned, net_name_scope_pruned+"_p"+str(scope_kept_percentage), v.op.name):
                                        v for v in get_model_variables_within_scopes(train_scopes)}
