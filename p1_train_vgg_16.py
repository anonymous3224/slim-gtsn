...
def main(_):
    ...
    # get the layer names inside a block 
    start_layer_id = FLAGS.block_size*FLAGS.block_id
    ...
    # get layer names to be pruned so that the pruned block can fit into the test network. 
    ...
    if FLAGS.block_id !=0:
        ...
        config_length +=1    
    # update block size, used to generate block configuration 
    # block_size = len(pruned_layer_names)
    ...
    results_dir = os.path.join(FLAGS.train_dir, 'm'+str(FLAGS.block_size)+'_b'+str(FLAGS.block_id)+'_'+block_config_str) 
    ...
    prune_scopes =[layer_name_to_prune_scope(layer_name, net_name_scope_checkpoint) for layer_name in pruned_layer_names]
    shorten_scopes = [layer_name_to_shorten_scope(layer_name, net_name_scope_checkpoint) for layer_name in pruned_layer_names]
    variables_init_value, pruned_filter_indexes = get_init_values_for_pruned_layers(prune_scopes, shorten_scopes, block_config)
    # print('HG: pruned_filter_indexes', pruned_filter_indexes)
    ...
    info += 'block_layer_names:'+ str(block_layer_names)+'\n'
    info += 'pruned_layer_names:'+ str(pruned_layer_names)+'\n'
    info += 'block_config:'+ str(block_config)+'\n'
    log_info = info+'\n'
    ...
    with tf.Graph().as_default():
        ...
        if FLAGS.block_id ==0:
            ...
            with tf.name_scope('inputs_selector'):
                ...
                print('HG: inputs selector:', original_inputs.get_shape().as_list(), ' to ', block_inputs.get_shape().as_list())
        ...
        if is_last_block:
            ...
            with tf.name_scope('output_selector'):
            ...
            for i in range(init_global_step_value, FLAGS.max_number_of_steps+1):
                ...
                if i > init_global_step_value: # FLAGS.max_number_of_steps:
                    ...
                    # record summary
                        ...
                        results = sess.run([summary_op, train_tensor])
                        train_summary, loss_value= results[0], results[-1]
                    ...
                    if i%FLAGS.log_every_n_steps==0 and duration_cnt > 0:
                        ...
                        info = ('%s: step %d, loss = %s (%.1f examples/sec; %.3f sec/batch)') % (datetime.now(), i, str(loss_value), examples_per_sec, sec_per_batch)
                        print(info)
                        duration = 0 
                        duration_cnt = 0 
                    ...
                    results = sess.run([summary_op, total_loss]) #loss_value = sess.run(total_loss)
                    train_summary, loss_value = results[0], results[-1]
                    train_writer.add_summary(train_summary, i)
