...
# prune all the valid layers and train the network with the objective being cross-entropy+kd+regularization. 

# train a network with both cross entropy and l2_loss on logits 
...
tf.app.flags.DEFINE_boolean(
    ...
    'if True, the loss function will contain the cross entropy.')

tf.app.flags.DEFINE_float(
    'logits_loss_lambda', 1, 'The weights to be applied on l2 logits loss.')
...
def main(_):
    ...
    indexed_prune_scopes, kept_percentage = config_to_indexed_prune_scopes(config, indexed_prune_scopes_for_units, kept_percentages)
    prune_info = indexed_prune_scopes_to_prune_info(indexed_prune_scopes, kept_percentage)
    
    # prepare file system 
    results_dir = os.path.join(FLAGS.train_dir, 'id'+str(FLAGS.configuration_index)) 
    ...
    with tf.Graph().as_default():
        ...
        # Define the model #
        ####################
        logits_teacher, end_points_teacher = network_fn(images, is_training=False)
        logits_train, end_points = network_fn_pruned(images, is_training=True, is_local_train=False, reuse_variables=False, scope = net_name_scope_pruned)
        ...
        regularization_losses = get_regularization_losses_within_scopes([net_name_scope_pruned+'/'])
        print_list('regularization_losses', regularization_losses)

        # get knowledge distillation loss: l2_loss on logits 
        if FLAGS.logits_loss_lambda > 0:
            logits_loss = add_l2_loss(logits_teacher, logits_train, weights=FLAGS.logits_loss_lambda)
        ...
        if not FLAGS.include_cross_entropy and FLAGS.logits_loss_lambda ==0:
            ...
            outputs_scope = get_prune_units_outputs_scope(last_indexed_prune_scope, net_name_scope_checkpoint)
            outputs = end_points_teacher[outputs_scope]

            outputs_scope = get_prune_units_outputs_scope(last_indexed_prune_scope, net_name_scope_pruned)
        ...
        # Add train operation       #
        #############################
        variables_to_train = get_trainable_variables_within_scopes([net_name_scope_pruned+'/'])
        ...
        update_ops = get_update_ops_within_scopes([net_name_scope_pruned+'/'])
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            if (not FLAGS.continue_training) or (not tf.train.latest_checkpoint(train_dir)):
                ...
                print_list("restore pruned model variables", variables_to_restore.values())
                load_checkpoint(sess, FLAGS.checkpoint_path, var_list=variables_to_restore)
