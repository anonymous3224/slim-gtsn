...
def main(_):
    ...
    print_list("indexed_prune_scopes", indexed_prune_scopes)

    net_name_scope_pruned = FLAGS.net_name_scope_pruned
    net_name_scope = FLAGS.net_name_scope
    net_name_scope_checkpoint = FLAGS.net_name_scope_checkpoint
    kept_percentages = [float(x) for x in FLAGS.kept_percentages.split(',')]
    print_list('kept_percentages', kept_percentages)

    # prepare file system 
    results_dir = os.path.join(FLAGS.train_dir, 'kp'+FLAGS.kept_percentages) #+'_'+str(FLAGS.max_number_of_steps))
    ...
    with tf.Graph().as_default():
        ...
        # Each pruned network contains totally 16/block_size train operations. 
        #print('HG: kept_percentages:' + str(len(kept_percentages)) + ', ' + str(kept_percentages))
        train_tensors = []
        total_losses = [] 
        ...
        for kept_percentage in kept_percentages:
            ...
            set_prune_units_inputs(prune_info, net_name_scope, end_points=end_points, block_size=FLAGS.block_size)
            # pprint(prune_info)
            # return 
            ...
            #############################
            outputs_scopes = get_prune_units_outputs_scopes(prune_info, net_name_scope, block_size=FLAGS.block_size)
            outputs_scopes_pruned = get_prune_units_outputs_scopes(prune_info, net_name_scope_pruned, block_size=FLAGS.block_size)
            train_scopes = get_train_scopes_for_local_train(prune_info, net_name_scope_pruned, block_size=FLAGS.block_size) 
            
            print_list('outputs_scopes:', outputs_scopes)
            print_list('train_scopes:', train_scopes)
            # return 
            for i in range(len(outputs_scopes)):
                # add l2 losses
                appendix = '_p'+str(kept_percentage)+'_'+str(i)
                collection_name = 'subgraph_losses'+appendix
                # print("HG: collection_name=", collection_name)

                outputs_scope = outputs_scopes[i]
                outputs_scope_pruned = outputs_scopes_pruned[i]
                ...
                train_scope = train_scopes[i] 
                # print('HG: train_scope:', train_scope)
                regularization_losses = get_regularization_losses_within_scopes(train_scope, add_to_collection=True, collection_name=collection_name)
                # print_list('regularization_losses('+ str(i)+')', regularization_losses)
                ...
                # Add train operation       #
                #############################
                variables_to_train = get_trainable_variables_within_scopes(train_scope)
                # print_list('variables_to_train('+ str(i)+')', variables_to_train)
                # add summary to variables_to_train 
                ...
                update_ops = get_update_ops_within_scopes(train_scope)
                # print_list('update_ops('+ str(i)+')', update_ops)
        ...
        print("HG: trainable_variables from "+net_name_scope+"=", len(get_trainable_variables_within_scopes([net_name_scope+'/'])))
        print("HG: trainable_variables from "+net_name_scope_pruned+"=", len(get_trainable_variables_within_scopes([net_name_scope_pruned+'/'])))
        print("HG: model_variables from "+net_name_scope+"=", len(get_model_variables_within_scopes([net_name_scope+'/'])))
        print("HG: model_variables from "+net_name_scope_pruned+"=", len(get_model_variables_within_scopes([net_name_scope_pruned+'/'])))
        print("HG: global_variables from "+net_name_scope+"=", len(get_global_variables_within_scopes([net_name_scope+'/'])))
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            if (not FLAGS.continue_training) or (not tf.train.latest_checkpoint(train_dir)):
                ...
                ###########################################
                variables_to_restore = {re.sub(net_name_scope, net_name_scope_checkpoint, v.op.name):
                                    v for v in  get_model_variables_within_scopes([net_name_scope+'/'])}
                print_list("restore model variables for original", variables_to_restore.values())
                load_checkpoint(sess, FLAGS.checkpoint_path, var_list=variables_to_restore)

                #################################################
                # Init  pruned networks  with  well-trained model #
                #################################################
                prune_scopes = indexed_prune_scopes_to_prune_scopes(indexed_prune_scopes, net_name_scope)
                shorten_scopes = indexed_prune_scopes_to_shorten_scopes(indexed_prune_scopes, net_name_scope) 
                modified_scopes = set(prune_scopes+shorten_scopes)
                print_list("prune_scopes", prune_scopes)
                print_list("shorten_scopes", shorten_scopes)
                for i in range(len(pruned_net_name_scopes)):
                    ...
                    reinit_scopes = sorted([re.sub(net_name_scope, net_name_scope_pruned, v) for v in modified_scopes])
                    variables_to_reinit = get_model_variables_within_scopes(reinit_scopes)
                    print_list("variables_to_reinit", variables_to_reinit)

                    kept_percentage = kept_percentages[i]
                    variables_init_value = get_pruned_kernel_matrix(sess, prune_scopes, shorten_scopes, kept_percentages=kept_percentage)
                    assign_ops = []
                    for v in variables_to_reinit:
                        key = re.sub(net_name_scope_pruned, net_name_scope, v.op.name)
